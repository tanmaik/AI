...
def currentPlayer(word):
    if (len(word) - len(originalWord)) % 3 == 2 or (len(word) - len(originalWord)) % 3 == 0:
        return "H"
    else:
        return "C"
...
def minStep(word):
    if gameIsOver(word):
        loses.add((word[len(originalWord):], currentPlayer(word)))
...
def maxMove(word):
    ...
    moves = []
    bad = []
    for ending, player in loses:
        if player == "C" and len(ending) >= 1:
            bad.append(ending[0])
    for letter in alphabet:
        if letter not in bad:
            for word, move in possibleNextMoves(originalWord):
                if letter == move:
                    moves.append(letter)
    return moves
